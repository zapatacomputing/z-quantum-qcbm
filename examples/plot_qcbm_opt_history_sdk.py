...
from zquantum.core.utils import get_ordered_list_of_bitstrings
...
for step_name in data:
    step_result = data[step_name].result
    step_input = data[step_name].inputs
    if step_name == "generate-target-distribution":
        number_of_qubits = len(list(step_result.distribution_dict.keys())[0])
        ordered_bitstrings = get_ordered_list_of_bitstrings(number_of_qubits)
        target_distribution = []
        for key in ordered_bitstrings:
            target_distribution.append(step_result.distribution_dict.get(key, 0))
