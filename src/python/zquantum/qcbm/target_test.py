...
from .target import ( get_num_bars_and_stripes_patterns,  get_bars_and_stripes_target_distribution,  bars_and_stripes_zigzag)
...
class TestTarget(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.percentages = [ 1., 0.75, 0.5, 0.25, 0.1 ]
        ...
        for method in self.methods:
            for i, grid in enumerate(self.grids):
                for percentage in self.percentages:
                    # When
                    bars_and_stripes_distribution = get_bars_and_stripes_target_distribution(grid[0], grid[1],  percentage=percentage, method=method)
                    
                    # Then
                    self.assertIsInstance(bars_and_stripes_distribution, BitstringDistribution)
                    expected_num_patters = max(int(self.number_of_patterns[i]*percentage), 1)
    ...
    def test_get_bars_and_stripes_target_distribution_unsupported_method(self):
        ...
        # When/Then
        self.assertRaises(RuntimeError, lambda: get_bars_and_stripes_target_distribution(nrows, ncols, percentage=1., method=method))
