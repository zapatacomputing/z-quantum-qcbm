...
class TestQCBMAnsatz(unittest.TestCase):
    ...
    def test_get_single_qubit_layer_wrong_num_params(self):
        ...
        params = np.ones(3)
        # When/Then
        self.assertRaises(AssertionError, lambda: get_single_qubit_layer(n_qubits, params, single_qubit_gates))
    ...
    def test_get_single_qubit_layer_one_gate(self):
        ...
        for n in n_qubits:
            ...
            # When
            circ = get_single_qubit_layer(n, params, single_qubit_gates)
    ...
    def test_get_single_qubit_layer_multiple_gates(self):
        ...
        for n in n_qubits:
            # Given
            params = [x for x in range(0, 3*n)]
            test = cirq.Circuit()
            qubits = [cirq.LineQubit(x) for x in range(0, n)]
            for i in range(0,n):
                test.append(cirq.Ry(params[i]).on(qubits[i]))
            for i in range(0,n):
                test.append(cirq.Rx(params[n+i]).on(qubits[i]))
            for i in range(0,n):
                test.append(cirq.Rz(params[2*n+i]).on(qubits[i]))
            u_cirq = test._unitary_()
    ...
    def test_get_all_topology(self):
        ...
        # When
        ent_layer = get_all_topology(n_qubits, params, static_entangler)
    ...
    def test_get_line_topology(self):
        ...
        # When
        ent_layer = get_line_topology(n_qubits, params, static_entangler)
    ...
    def test_get_entangling_layer_toplogy_supported(self):
        ...
        for n in n_qubits:
            # Given
            params = np.zeros((int((n*(n-1))/2)))
            all_topology_layer = get_all_topology(n, params, static_entangler)
            # When
            entangling_layer = get_entangling_layer(n, params, static_entangler, topology)
            # Then
        ...
        for n in n_qubits:
            # Given
            params = np.zeros((n-1))
            line_topology_layer = get_line_topology(n, params, static_entangler)
            # When
            entangling_layer = get_entangling_layer(n, params, static_entangler, topology)
            # Then
    ...
    def test_build_qcbm_circuit_iontrap_three_layers(self):
        ...
        expected_second_layer = get_entangling_layer(n_qubits, params[1], "XX", topology)
        expected_pycircuit = Program()
        for i in range(n_qubits):
